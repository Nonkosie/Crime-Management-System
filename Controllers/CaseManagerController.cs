...
namespace CrimeManagement.Controllers
{
    ...
    public class CaseManagerController : Controller
    {
        ...
        [Authorize(Roles = "Captain, Case Manager, Police Officer")]
        public async Task<IActionResult> Index()
        {
            ...
            if (user != null)
            {
                ...
                if (User.IsInRole("Police Officer"))
                {

                    var caseManagers = await _userManager.GetUsersInRoleAsync("Case Manager");
                    return View(caseManagers);
                }
                else if (User.IsInRole("Case Manager"))
                {
                    // Retrieve only the currently signed-in Case Manager
                    var caseManager = await _userManager.FindByIdAsync(user.Id);
                    if(caseManager != null)
                    {
                        var caseManagers = new List<ApplicationUser> { caseManager };
                        return View(caseManagers);
                    } 
                }
